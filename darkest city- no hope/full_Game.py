...
pygame.display.set_caption("Darkest City: No Hope")
win_screen = pygame.image.load('yOU_WiN meme.png')
...
# x location, y location, img width, img height, img file
    ...
    class Hood(pygame.sprite.Sprite):
        ...
        def __init__(self, x, y, img):
            pygame.sprite.Sprite.__init__(self)
            self.image = pygame.image.load('hood_of_car2.png')
            self.image.convert_alpha()
            self.image.set_colorkey(ALPHA)
            self.rect = self.image.get_rect()
            self.rect.x = x
    ...
    class Wheels(pygame.sprite.Sprite):
        ...
        def __init__(self, x, y, img):
            pygame.sprite.Sprite.__init__(self)
            self.image = pygame.image.load('wheels2.png')
            self.image.convert_alpha()
            self.image.set_colorkey(ALPHA)
            self.rect = self.image.get_rect()
            self.rect.x = x
    ...
    class Level:
        ...
        def platform(lvl, tx, ty):
            ...
            if lvl == 1:
                ...
                while i < len(ploc):
                    j = 0
                    while j <= ploc[i][2]:
                        plat = Platform((ploc[i][0] + (j * tx)), ploc[i][1], tx, ty, 'piskel_platform.png')
                        plat_list.add(plat)
                        j = j + 1
        ...
        def loot(lvl, hloc):
            if lvl == 1:
                backdoor = Backdoor(hloc[0], hloc[1], 'car_back_door2.png')
        ...
        def lootb(lvl, zloc):
            if lvl == 1:
                headlight = Headlights(zloc[0], zloc[1], 'headlights_2.png')
        ...
        def lootc(lvl, mloc):
            if lvl == 1:
                hood = Hood (mloc[0], mloc[1], 'hood_of_car2.png')
        ...
        def lootd(lvl, vloc):
            if lvl == 1:
                wheels = Wheels(vloc[0], vloc[1], 'wheels2.png')
        ...
        def loote(lvl, qloc):
            if lvl == 1:
                engine = Engine(floc[0], floc[1], 'engine4.png')
    ...
    while main:
        ...
        if player.score == 6:
            backdrop = pygame.image.load('yOU_WiN meme.png')
            world.fill(WHITE)
            ...
            backdrop = pygame.image.load('you_died_restart1.png')
            world.fill(BLACK)
            world.blit(backdrop, backdropbox)
            player.update()
            player.gravity()
